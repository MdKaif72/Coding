...
public class TimeBasedKeyValueStore {
    ...
    class TimeMap {
        ...
        public String get(String key, int timestamp) {
            if (!map.containsKey(key))
                return "";
            Optional<TimeStampValue> opt = binarySearch(map.get(key), timestamp);
            return opt.isEmpty() ? "" : opt.get().value;


        }
    }

    private Optional<TimeStampValue> binarySearch(List<TimeStampValue> arr, int timeStamp) {
